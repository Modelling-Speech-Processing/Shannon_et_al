...
The heatmap in Figure 3 is produced by the Plot_concordance_coefficient_heatmaps.jl script.

For the second half of the paper, figures 5, 6, 7, 8, 9, and 9_1:

A further source file Phase_Concentration_Metric.jl contains code to run the PCM tests, which the script Compute_and_plot_PCM.jl uses to generate figure 5. Note, due to the use of sine wave stimuli, generated by Generate_Sine_Drives.jl, that are centred at 0, there are new alternate versions of the oscillator and NGNMM models in NGNMM_NSP_paper_code.jl with the name extended by "plus1_for_sine" that offset the drive by +1 making it strictly positive.

Figure 6 data (but not the figure) is produced by the Vary_Stimulus_Rate.jl script, in particular the second half that runs the test across all syllable conditions. This takes a while to run, roughly 30 minutes on an M1 pro macbook, our results data from this test is therefore included, see below.

Figure 7 and 8 are produced by the Plot_phase_reset_ripples.jl script. Figure 9 by the Plot_IEPC.jl script, and Figure 9_1 by the Plot_IEPC_ringing_after_end_of_stimulus.jl script. 
...
Figure 6 data is included in csv format in ./Results/Speech_rate_test. These files are what are used by the Plot_vary_stim_rate.jl script to produce figure 6.
